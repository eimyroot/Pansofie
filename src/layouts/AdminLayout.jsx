  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside className="hidden lg:flex fixed inset-y-0 left-0 z-40 w-64 flex-col border-r border-border/60 bg-card">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV.map(({ to, label, icon: Icon, end }) => {
            ...
            return (
              <Link key={to} to={to} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${active ? "bg-foreground/10 text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted/60"}`}>
      ...
      <div className="lg:pl-64">
        <div className="lg:hidden h-14 flex items-center gap-2 px-5 border-b border-border/60 sticky top-0 bg-background/90 backdrop-blur z-30">
          <span className="h-7 w-7 rounded-lg bg-foreground text-background flex items-center justify-center"><Leaf size={15} strokeWidth={2.4} /></span>
          <span className="font-heading font-bold">Pansofie Admin</span>
          <button type="button" onClick={handleLogout} className="ml-auto text-xs text-muted-foreground">Odhlásit</button>
        </div>
        <main className="px-5 sm:px-8 lg:px-12 py-8 max-w-7xl mx-auto pb-24 lg:pb-8"><Outlet /></main>
      </div>

      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-card/95 backdrop-blur-md border-t border-border/70 overflow-x-auto no-scrollbar">
        <div className="flex items-stretch min-w-max">
          {NAV.map(({ to, label, icon: Icon, end }) => {
            const active = end ? location.pathname === to : location.pathname === to || location.pathname.startsWith(to + "/");
            return <Link key={to} to={to} className={`flex flex-col items-center gap-0.5 px-4 py-2.5 text-[11px] font-medium whitespace-nowrap ${active ? "text-foreground" : "text-muted-foreground"}`}><Icon size={18} strokeWidth={active ? 2.4 : 2} /> {label}</Link>;
          })}
        </div>
      </nav>
    </div>
