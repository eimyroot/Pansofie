...
export default function ExperienceComposer({ role, goal, contributions = [], problem, participants = [], step = 0 }) {
  ...
  return (
    <aside aria-label="Živý náhled vznikající Experience" className="rounded-[2rem] border border-border bg-background/90 p-5 sm:p-6 shadow-sm lg:sticky lg:top-28">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Živý Experience Composer</p>
          <h3 className="mt-2 text-xl sm:text-2xl font-semibold font-display">Pansofie se skládá podle vašich voleb.</h3>
        </div>
      ...
      <div className="mt-5 space-y-2.5">
        ...
        <Signal ready={Boolean(problem)} icon={Route} label="Na čem vznikne Experience" value={problem?.challenge} />
        <Signal ready={selectedPeople.length >= 2} icon={UsersRound} label="Kdo je kolem ní" value={selectedPeople.length ? selectedPeople.join(" · ") : ""} />
      </div>
      ...
      <div className="mt-5 rounded-2xl bg-foreground p-5 text-background">
        ...
        <p className="mt-4 text-sm leading-relaxed text-background/75">
          {step < 3 && "Nejdřív skládáme smysl vztahu. Teprve potom vybíráme konkrétní problém."}
          {step === 3 && "Teď se abstraktní záměr mění na skutečný problém, ze kterého může vzniknout Experience."}
          {step === 4 && "Přidáváme lidi jen tam, kde mají jasný účel a bezpečnou roli."}
          {step >= 5 && "Experience je složená. Výsledek teď ukazuje cestu od problému přes důkaz a reflexi až k Passportu a dalšímu kroku."}
        </p>
      </div>
    </aside>
  );
