      ...
      <div className="container-px max-w-7xl mx-auto h-[74px] flex items-center justify-between gap-4">
        <Link to="/" className="group flex items-center gap-2.5" aria-label="Pansofie — domů">
          ...
          <span>
            ...
            <span className="hidden md:block mt-1 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Experience-first learning</span>
          </span>
        </Link>

        <nav className="hidden xl:flex items-center gap-1 text-sm" aria-label="Veřejná navigace">
          {links.map(([to, label]) => (
            <a key={label} href={to} className="rounded-xl px-3 py-2 text-muted-foreground hover:text-foreground hover:bg-card transition-colors">
              {label}
            </a>
          ))}
        </nav>
        ...
        <button className="xl:hidden h-10 w-10 rounded-xl border border-border bg-card flex items-center justify-center" onClick={() => setOpen((value) => !value)} aria-label={open ? "Zavřít menu" : "Otevřít menu"} aria-expanded={open}>
