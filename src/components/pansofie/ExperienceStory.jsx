...
const STEPS = [
  ...
  { number: "06", title: "Passport", text: "Po ověřeném dokončení zůstává soukromý záznam skutečné zkušenosti a jejího dalšího kroku.", icon: GraduationCap },
];
      ...
      <div className="container-px max-w-7xl mx-auto">
        <div className="grid grid-cols-1 xl:grid-cols-[0.72fr_1.28fr] gap-10 xl:gap-14 items-start">
          <div className="xl:sticky xl:top-28">
            ...
            <p className="mt-5 text-lg text-muted-foreground leading-relaxed">Pilotní Experience <strong className="text-foreground font-semibold">Zlepši svou školu</strong> ukazuje princip Pansofie od potřeby až po ověřený Passport.</p>
            <div className="mt-6 rounded-2xl border border-border bg-background p-5 flex items-start gap-3"><ShieldCheck size={20} className="text-primary shrink-0 mt-0.5" /><p className="text-sm text-muted-foreground leading-relaxed">Toto je <strong className="text-foreground font-semibold">pilotní scénář</strong>, ne zveřejněná případová studie. Reálné výsledky doplníme až po skutečném field pilotu.</p></div>
            <div className="mt-7 flex flex-col sm:flex-row sm:items-center gap-3">
              <Link to="/zapojit-se?mode=simulator" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 font-semibold text-primary-foreground hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">Vyzkoušet Pansofii za 60 sekund <ArrowRight size={17} /></Link>
              <Link to="/pilot" className="inline-flex items-center gap-2 text-primary font-semibold hover:opacity-80">Prozkoumat pilot <ArrowRight size={16} /></Link>
            </div>
          </div>
              ...
              {STEPS.map((step) => {
                ...
                return (
                  <article key={step.number} className="relative rounded-3xl border border-border bg-background p-6 sm:p-7 min-h-56 flex flex-col">
                    <div className="flex items-start justify-between gap-4"><span className="h-12 w-12 rounded-2xl bg-primary/10 text-primary flex items-center justify-center relative z-10"><Icon size={22} /></span><span className="text-xs font-semibold text-muted-foreground">{step.number}</span></div>
