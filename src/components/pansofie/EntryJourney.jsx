...
function ChoiceCard({ selected, onClick, children, icon: Icon, disabled = false, multi = false }) {
  return (
    <button
      ...
      className={`group relative w-full text-left rounded-2xl border p-4 sm:p-5 transition-colors motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${selected ? "border-primary/50 bg-primary/[0.065]" : "border-border bg-background hover:bg-card"} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
    >
...
export default function EntryJourney() {
  ...
  return (
    <div className="container-px max-w-7xl mx-auto py-10 sm:py-16">
      ...
      <section className="mt-7 max-w-5xl mx-auto rounded-[2rem] border border-border bg-card/35 p-5 sm:p-8" aria-live="polite">
        {step === 0 && (
          <div>
        ...
        {step === 4 && (
          <div>
            ...
            <div className="mt-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{PARTICIPANTS.map(([id, label]) => <ChoiceCard key={id} selected={participants.includes(id)} onClick={() => toggleParticipant(id)} disabled={id === "learner" || id === "teacher"} multi>{label}{(id === "learner" || id === "teacher") && <span className="block mt-1 text-xs font-normal text-muted-foreground">povinný bezpečný základ</span>}</ChoiceCard>)}</div>
          </div>
        )}
        ...
        {step === 5 && role && problem && (
          <div>
            ...
            <div className="mt-8">
              <p className="text-xs font-semibold uppercase tracking-wide text-primary">Živá Experience</p>
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">{path.map(([number, title, text]) => <div key={number} className="rounded-2xl border border-border bg-background p-4 flex gap-4"><span className="h-9 w-9 shrink-0 rounded-xl bg-primary/10 text-primary text-xs font-semibold flex items-center justify-center">{number}</span><div><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-xs sm:text-sm text-muted-foreground leading-relaxed">{text}</p></div></div>)}</div>
            </div>
