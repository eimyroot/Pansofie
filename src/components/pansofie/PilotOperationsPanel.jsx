...
import PilotReadinessPanel from "@/components/pansofie/PilotReadinessPanel";
import {
...
export default function PilotOperationsPanel({ teacherMemberships = [], learners = [], missions = [], onChanged }) {
  ...
  const selectedCohort = cohorts.find((item) => item.id === form.cohortId) || null;
  ...
  return (
    <section className="mb-12 rounded-[2rem] border border-primary/20 bg-primary/[0.025] p-5 sm:p-7">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-primary"><Layers3 size={18} /><p className="text-xs font-semibold uppercase tracking-wide">Field Pilot Operations</p></div>
          <h2 className="mt-2 text-2xl font-semibold font-heading">Kohorta → tým → 3 Experiences → readiness</h2>
          <p className="mt-2 text-sm text-muted-foreground max-w-3xl leading-relaxed">Tým sdílí kontext a výstup, ale každý žák dál dostává vlastní run, vlastní reflexi a vlastní Passport. Field pilot zůstává planned, dokud R2 readiness není PASS.</p>
        </div>
      ...
      <div className="mt-7 grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <label className="text-sm"><span className="block font-medium mb-2">Kohorta</span><select value={form.cohortId} onChange={(e) => handleCohortChange(e.target.value)} className="w-full rounded-xl border border-border bg-background px-3 py-2.5"><option value="">— zatím žádná —</option>{visibleCohorts.map((cohort) => <option key={cohort.id} value={cohort.id}>{cohort.name} · {cohort.status}</option>)}</select></label>
      ...
      <div className="mt-5 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-border bg-background p-4 sm:p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">1. Založit bounded kohortu</p>
          <input value={form.cohortName} onChange={(e) => setForm((current) => ({ ...current, cohortName: e.target.value }))} className="mt-3 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm" placeholder="Název kohorty" />
          <div className="mt-2 grid grid-cols-2 gap-2"><label className="text-xs text-muted-foreground">Začátek<input type="date" value={form.startsOn} onChange={(e) => setForm((current) => ({ ...current, startsOn: e.target.value }))} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-foreground" /></label><label className="text-xs text-muted-foreground">Konec<input type="date" value={form.endsOn} onChange={(e) => setForm((current) => ({ ...current, endsOn: e.target.value }))} className="mt-1 w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm text-foreground" /></label></div>
          <button type="button" disabled={busy || !form.organizationId || !form.cohortName.trim() || !form.startsOn || !form.endsOn} onClick={() => run(async () => { const id = await createPilotCohort({ organizationId: form.organizationId, name: form.cohortName.trim(), startsOn: form.startsOn, endsOn: form.endsOn }); setForm((current) => ({ ...current, cohortId: id })); }, "Kohorta byla založena jako planned s fixním plánem 3 Experiences.")} className="mt-3 rounded-xl bg-foreground text-background px-4 py-2.5 text-sm font-semibold disabled:opacity-50 inline-flex items-center justify-center gap-2"><Plus size={15} /> Založit kohortu</button>
        </div>
      ...
      <PilotReadinessPanel cohort={selectedCohort} onChanged={load} />
    </section>
  );
}
