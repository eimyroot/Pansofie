...
export default function CoreWorkspace({ role = "owner" }) {
  ...
  return <div className="core-workspace">
    ...
    <div className="workspace-body">
      ...
      {active === "Labs" && <section className="labs-world"><div className="section-heading"><div><p className="eyebrow">Bezpečný prostor pro pokusy</p><h2>Pansofie Labs</h2></div></div><div className="lab-feature"><div><p className="eyebrow">Green Hope</p><h3>Planeta jako učebna</h3><p>Nejen o ekologii mluvit, ale pěstovat, kompostovat, měřit, opravovat a pečovat.</p><div className="topic-cloud">{GREEN_HOPE_TOPICS.map((topic) => <span key={topic}>{topic}</span>)}</div></div><Image src="/art/pansofie-v1/hero-tree.webp" alt="Příroda jako prostor pro praktické učení" width={620} height={420}/></div><div className="lab-panel"><p className="eyebrow">Urban Family Farm</p><h3>Od semínka k hodnotě</h3><div className="cycle-flow">{URBAN_FARM_CYCLE.map((step, index) => <span key={step}><b>{index + 1}</b>{step}</span>)}</div></div><div className="lab-panel"><p className="eyebrow">Family Team</p><h3>Rodina tvoří společně</h3><div className="family-mission-grid">{FAMILY_MISSIONS.map((mission) => <span key={mission}>{mission}</span>)}</div></div></section>}
