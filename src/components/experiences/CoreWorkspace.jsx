    ...
    <div className="workspace-body">
      ...
      {active === "Přehled" && <>
        <section className="workspace-hero">
          ...
          <Image src={isFamily ? "/assets/current/photos/community-garden.webp" : "/assets/current/photos/community-garden.webp"} alt="Pansofie propojuje učení s reálným světem" width={760} height={560} priority />
        </section>
      ...
      {active === "Labs" && <section className="labs-world"><div className="section-heading"><div><p className="eyebrow">Bezpečný prostor pro pokusy</p><h2>Pansofie Labs</h2></div></div><div className="lab-feature"><div><p className="eyebrow">Green Hope</p><h3>Planeta jako učebna</h3><p>Nejen o ekologii mluvit, ale pěstovat, kompostovat, měřit, opravovat a pečovat.</p><div className="topic-cloud">{GREEN_HOPE_TOPICS.map((topic) => <span key={topic}>{topic}</span>)}</div></div><Image src="/assets/current/photos/volunteer-garden.webp" alt="Příroda jako prostor pro praktické učení" width={620} height={420}/></div><div className="lab-panel"><p className="eyebrow">Urban Family Farm</p><h3>Od semínka k hodnotě</h3><div className="cycle-flow">{URBAN_FARM_CYCLE.map((step, index) => <span key={step}><b>{index + 1}</b>{step}</span>)}</div></div><div className="lab-panel"><p className="eyebrow">Family Team</p><h3>Rodina tvoří společně</h3><div className="family-mission-grid">{FAMILY_MISSIONS.map((mission) => <span key={mission}>{mission}</span>)}</div></div></section>}
      ...
      {active === "Portfolio" && <section><div className="section-heading"><div><p className="eyebrow">Evidence · reflexe · růst</p><h2>Moje cesta v čase</h2></div></div><div className="portfolio-layout"><div className="growth-tree"><Image src="/assets/current/photos/home-community.webp" alt="Vizuální strom osobního rozvoje" width={620} height={620}/><strong>Level 5</strong><span>850 / 1 000 XP</span></div><div className="reflection"><h3>Co se mi povedlo?</h3><p>Reflexe je soukromá, dokud se ji sami nerozhodnete sdílet.</p><textarea aria-label="Soukromá reflexe" placeholder="Zapište si, co jste objevili…"/><button>Uložit soukromě</button></div><ImpactCard /></div></section>}
