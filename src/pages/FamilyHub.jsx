...
export default function FamilyHub() {
  ...
  if (loading) {
    return <div className="px-5 sm:px-8 lg:px-12 py-8 max-w-6xl mx-auto"><div className="card-soft p-6 text-sm text-muted-foreground">Načítám Family workspace…</div></div>;
  }
  ...
  return (
    <div className="px-5 sm:px-8 lg:px-12 py-8 max-w-6xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 text-primary"><HeartHandshake size={20} /><span className="text-sm font-semibold">PANSOFIE FAMILY · FIELD PILOT R3</span></div>
        ...
        <button type="button" onClick={load} disabled={busy} className="shrink-0 inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium disabled:opacity-50"><RefreshCw size={15} /> Obnovit</button>
      </header>
      ...
      <section className="mb-8 rounded-3xl border border-primary/20 bg-primary/[0.035] p-5 sm:p-6">
        <div className="flex items-start gap-3">
          <ShieldCheck size={20} className="text-primary shrink-0 mt-0.5" />
          <div>
            <h2 className="font-semibold">Privacy boundary</h2>
            <p className="text-sm text-muted-foreground mt-1">Rodina nevidí raw evidence ani soukromou reflexi. Verified guardian relationship sama o sobě nestačí: pro aktivní Experience je nutný purpose-specific <code>guardian_family_participation</code>; Passport summary má oddělený <code>guardian_passport_view</code>.</p>
          </div>
        </div>
      </section>
      ...
      {guardianAccess ? (
        <>
          <section className="mb-10">
            <div className="flex items-center gap-2 mb-4"><UsersRound size={18} className="text-primary" /><h2 className="text-xl font-semibold font-heading">Aktuální Experience</h2></div>
            {contexts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {contexts.map((item) => (
                  <article key={item.run_id} className={`card-soft p-5 ${form.runId === item.run_id ? "border-primary/35" : ""}`}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="rounded-full bg-muted px-2.5 py-1 text-xs text-muted-foreground">{RUN_STATUS[item.run_status] || item.run_status}</span>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      <div className="rounded-xl border border-border bg-background p-3"><p className="text-xs font-semibold text-primary">PROČ</p><p className="mt-1 text-muted-foreground">{item.mission_why || "Účel je součástí zadání Experience."}</p></div>
                      <div className="rounded-xl border border-border bg-background p-3"><p className="text-xs font-semibold text-primary">JAK MŮŽE RODINA POMOCT</p><p className="mt-1 text-muted-foreground">{item.contribution_prompt || "Přidejte kontext, kontakt, zdroj nebo pozorování — neřešte úkol za žáka."}</p></div>
                    </div>
                    <p className="mt-3 text-xs text-muted-foreground">Pilotní okno: {item.pilot_starts_on || "—"} → {item.pilot_ends_on || "—"}</p>
                    <button type="button" onClick={() => setForm((current) => ({ ...current, runId: item.run_id }))} className="mt-4 rounded-xl border border-border px-4 py-2 text-sm font-semibold">Přidat rodinný podnět</button>
                  </article>
                ))}
              </div>
            )}
          ...
          {selectedContext && (
            <section className="mb-10 rounded-3xl border border-primary/25 bg-card p-5 sm:p-6">
              <div className="flex items-center gap-2"><Send size={18} className="text-primary" /><h2 className="text-xl font-semibold font-heading">Přispět do Experience</h2></div>
              ...
              <form onSubmit={submitContribution} className="mt-5 grid grid-cols-1 lg:grid-cols-[220px_1fr_auto] gap-3 items-end">
                ...
                <button disabled={busy || !form.content.trim()} className="rounded-xl bg-primary text-primary-foreground px-5 py-2.5 font-semibold disabled:opacity-50">Předat</button>
              </form>
            </section>
          )}

          <section className="mb-10">
            <div className="flex items-center gap-2 mb-4"><HeartHandshake size={18} className="text-primary" /><h2 className="text-xl font-semibold font-heading">Moje rodinné podněty</h2></div>
            {myContributions.length === 0 ? <div className="card-soft p-6 text-sm text-muted-foreground">Zatím nebyl předán žádný rodinný podnět.</div> : <div className="space-y-3">{myContributions.map((item) => <div key={item.id} className="card-soft p-4 flex flex-col sm:flex-row sm:items-start gap-3"><div className="flex-1"><p className="text-xs font-semibold text-primary">{KIND_LABEL[item.contribution_kind] || item.contribution_kind} · {item.status}</p><p className="text-sm mt-2 whitespace-pre-wrap">{item.content}</p><p className="text-[11px] text-muted-foreground mt-2">{new Date(item.created_at).toLocaleString("cs-CZ")}</p></div>{item.status === "active" && <button type="button" disabled={busy} onClick={() => withdraw(item.id)} className="rounded-xl border border-border px-3 py-2 text-xs font-medium disabled:opacity-50">Stáhnout</button>}</div>)}</div>}
          </section>
