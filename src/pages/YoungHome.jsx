...
function YoungTree() {
  return (
    <svg className="young-illustrated-tree" viewBox="0 0 520 500" role="img" aria-label="Kreslený strom Pansofie s lidmi, knihami a přírodou">
      <defs>
        <radialGradient id="leafA" cx="40%" cy="35%"><stop offset="0" stopColor="#dfe9a7"/><stop offset="1" stopColor="#78934f"/></radialGradient>
        <radialGradient id="leafB" cx="40%" cy="35%"><stop offset="0" stopColor="#f1e7a1"/><stop offset="1" stopColor="#93a65e"/></radialGradient>
        <linearGradient id="trunk" x1="0" x2="1"><stop stopColor="#9b6e3d"/><stop offset=".5" stopColor="#c08a4c"/><stop offset="1" stopColor="#7a512f"/></linearGradient>
        <filter id="soft"><feGaussianBlur stdDeviation=".8"/></filter>
      </defs>
      <ellipse cx="260" cy="455" rx="205" ry="24" fill="#dfe5c9" opacity=".75"/>
      <path d="M248 448c4-75 3-140-12-201-8-34-5-69 12-98 8 20 17 42 18 64 12-34 32-64 58-88-14 37-24 75-22 111 29-24 60-39 98-48-35 23-67 50-90 84-11 16-18 35-17 56 2 41 8 80 13 120z" fill="url(#trunk)"/>
      ...
      {[[130,190,70,"A"],[205,130,85,"B"],[302,123,92,"A"],[390,180,78,"B"],[92,260,60,"B"],[176,242,72,"A"],[352,250,75,"A"],[430,263,57,"B"],[250,78,66,"A"]].map(([x,y,r,t],i)=><circle key={i} cx={x} cy={y} r={r} fill={`url(#leaf${t})`} opacity=".96"/>)}
...
function YoungHeader() {
  return <header className="young-classic-header"><Link to="/young" className="young-classic-brand"><span className="young-mark"><Leaf size={18}/></span><span><strong>Pansofie</strong><small>Živý prostor, kde se hodnota vrací do oběhu.</small></span></Link><nav><a href="#jak">Jak to funguje</a><a href="#pro-koho">Pro koho</a><Link to="/materialovy-most">Koloběh</Link><Link to="/knihovna">Knihovna</Link><Link to="/o-projektu">Vize</Link></nav><div className="young-classic-actions"><Link to="/">Dospělá Pansofie</Link><Link className="young-join" to="/zapojit-se">Přidejte se</Link></div></header>;
}

function YoungFooter(){return <footer className="young-classic-footer"><div className="young-footer-principles"><p><b>VŠEM</b><span>Poznání a příležitosti mají být srozumitelné a dostupné bez zbytečných bariér.</span></p><p><b>VŠEMU</b><span>Technologie a vědění mají sloužit lidem, přírodě, kultuře i běžnému životu.</span></p><p><b>VŠESTRANNĚ</b><span>Rozum, vztahy, praktická dovednost a odpovědnost patří k sobě.</span></p></div><div className="young-footer-grid"><div><h3>Pansofie</h3><p>Poznávat v souvislostech. Růst celý život. Zlepšovat svět kolem sebe.</p><Link to="/zapojit-se">Najít svůj vstup →</Link></div><div><b>OBJEVOVAT</b><Link to="/jak-funguje">Jak to funguje</Link><Link to="/pro-koho">Pro koho</Link><Link to="/knihovna">Knihovna</Link><Link to="/o-projektu">Vize</Link></div><div><b>KOLOBĚH</b><Link to="/materialovy-most">Digitální kompost</Link><Link to="/materialovy-most">Mapa koloběhu</Link><Link to="/pro-koho/skoly">Školy & organizace</Link><Link to="/partneri">Lidé & mentoring</Link></div><div><b>PANSOFIE DNES</b><span>Pansofia · Vševěda</span><span>Pampaedia · Vševýchova</span><span>Panorthosia · Všenáprava</span><a href="#rozvoj">Co se může rozvíjet</a></div></div><div className="young-footer-thread"><span>ROZHLÉDNOUT SE</span><i/><span>PŘIPOJIT SE</span><i/><span>UDĚLAT MALÝ KROK</span><i/><span>POSLAT HODNOTU DÁL</span><i/><span>DALŠÍ MOŽNOST</span></div><div className="young-footer-bottom"><span>Prototyp překládající Pansofii, Pampaedii a Panorthosii do jednoduché současné komunitní zkušenosti.</span><span>© 2026 Pansofie · Příležitost, ne povinnost.</span></div></footer>}

export default function YoungHome(){return <div className="young-classic"><YoungHeader/><main><section className="young-classic-hero"><div className="young-hero-copy"><span>PANSOFIE PRO DNEŠNÍ DOBU</span><h1>Pansofie.<br/><em>Místo, kde se<br/>hodnota vrací<br/>zpět.</em></h1><p>Propojujeme lidi, znalosti a zdroje, aby se dobro, vědění i materiály mohly šířit, růst a znovu přinášet užitek.</p><div><Link className="young-main-cta" to="/zapojit-se">Přidejte se k nám <Leaf size={16}/></Link><a className="young-outline-cta" href="#jak">Podívejte se, jak to funguje <ArrowRight size={16}/></a></div></div><div className="young-tree-stage"><YoungTree/><span className="young-tree-badge b1">Vševěda<small>Pozorujeme svět v souvislostech.</small></span><span className="young-tree-badge b2">Vševýchova<small>Růst a učit se po celý život.</small></span><span className="young-tree-badge b3">Všenáprava<small>Zlepšovat svět kolem sebe.</small></span></div></section>

<section id="jak" className="young-panel young-actions-panel"><h2>Co byste dnes chtěli dát do pohybu?</h2><p>Vyberte si směr, který vás dnes volá. Bez závazků. Podle vás.</p><div className="young-five-actions">{ACTIONS.map(({icon:Icon,title,text,to,tone})=><Link key={title} to={to} className={`young-action ${tone}`}><span className="young-action-art"><Icon size={46}/></span><h3>{title}</h3><p>{text}</p><b><ArrowRight size={17}/></b></Link>)}</div></section>

<section id="pro-koho" className="young-panel young-pillars-panel"><h2>Tři pilíře Pansofie</h2><p>Komenského myšlenky přeložené do dnešního života.</p><div className="young-three-pillars">{PILLARS.map(({no,title,latin,icon:Icon,text,points})=><article key={title}><div className="young-pillar-art"><Icon size={58}/><span className="young-organic-orbit"/></div><div><strong>{no}</strong><h3>{title}</h3><em>{latin}</em><p>{text}</p><ul>{points.map(x=><li key={x}>{x}</li>)}</ul><Link to="/o-projektu">Zjistit více <ArrowRight size={14}/></Link></div></article>)}</div></section>

<section id="rozvoj" className="young-growth-panel"><span>ROZVOJ BEZ HODNOCENÍ</span><div><h2>Co se může cestou rozvíjet</h2><p>Nejsou to body, známky ani žebříček člověka. Jsou to oblasti, kterých se dobrovolná zkušenost může přirozeně dotknout.</p><div className="young-growth-chips">{GROWTH.map(x=><b key={x}>{x}</b>)}</div></div></section><section className="young-gentle-note"><Sprout size={24}/><p><strong>Pansofie nic nepřikazuje.</strong><span>Ukazuje možnosti. Můžete se jen rozhlédnout, nechat se inspirovat — a zapojit se teprve ve chvíli, kdy sami budete chtít.</span></p></section></main><YoungFooter/></div>}
