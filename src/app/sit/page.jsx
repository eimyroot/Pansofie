...
const ENTRY=[
 {title:"Rodina",text:"První tým, ve kterém se zkušenost, péče a odpovědnost potkávají.",href:"/komunita",image:pansofiePhoto("growing-together-16x9"),label:"BLÍZKÝ KONTEXT"},
 {title:"Škola",text:"Bezpečný kontext pro projekty, mise a mezioborové učení.",href:"/pro-skoly",image:pansofieScene("school-life-learning"),label:"UČENÍ"},
 {title:"Komunita",text:"Místní potřeby, péče o místo a spolupráce kolem konkrétního účelu.",href:"/komunita",image:pansofiePhoto("community-city-16x9"),label:"MÍSTO"},
 {title:"Organizace",text:"Know-how, materiál, prostor nebo kapacita navázaná na ověřený projekt.",href:"/pro-organizace",image:pansofieScene("organization-network"),label:"ZDROJE"},
];
export default function NetworkPage(){return <PublicShell active="/sit">
 ...
 <section className="pw-community-entry-grid">{ENTRY.map(card=><Link href={card.href} className="pw-community-entry-card" key={card.title}><div><Image src={card.image} alt="" fill sizes="(max-width:900px) 100vw,25vw"/></div><span>{card.label}</span><h2>{card.title}</h2><p>{card.text}</p></Link>)}</section>
 <EditorialFeatureBand eyebrow="KOMUNITA, NE FEED" title="Silná síť nevzniká počtem kontaktů. Vzniká tím, že si lidé umějí konkrétně pomoct." text="Rodina, třída, místní firma, senior, spolek nebo městská iniciativa mohou být součástí stejného projektu, aniž by se z Pansofie stala veřejná sociální síť lidí a jejich poloh." image={pansofiePhoto("growing-together-16x9")} imageAlt="Mezigenerační spolupráce lidí na společné činnosti" items={[["Rodiny","Společné mise při zachování jednotlivých identit."],["Školy","Bezpečné třídy a projektové kontexty."],["Senioři","Zkušenost a praktická pomoc bez veřejného katalogu osob."],["Organizace","Zdroje a know-how kolem konkrétního projektu."]]} link={{href:"/pro-organizace",label:"Zapojení organizací"}}/>
