-- PANSOFIE staging security follow-up.
--
-- PostgreSQL grants EXECUTE on new functions to the pseudo-role PUBLIC by
-- default. Revoking only from anon/authenticated is insufficient while PUBLIC
-- retains the grant, because both roles inherit PUBLIC privileges.
--
-- This migration removes PUBLIC EXECUTE from all PANSOFIE functions and then
-- restores only the explicitly intended authenticated entrypoints/helpers.
...
-- Only these helpers/RPCs are intentionally executable by authenticated users.
-- Helpers are invoked by RLS policies as the authenticated caller; governed
-- RPCs are called by the current frontend and contain their own authorization
-- checks.
