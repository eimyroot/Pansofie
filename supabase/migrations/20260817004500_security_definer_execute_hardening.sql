-- PANSOFIE staging security hardening discovered by Supabase database advisors.
--
-- Supabase exposes functions in the public schema through PostgREST RPC. Newly
-- created functions may retain EXECUTE privileges that are broader than the
-- application intends. This migration makes the intended boundary explicit.
--
-- Design:
-- - anon may execute NONE of the PANSOFIE SECURITY DEFINER/RPC helpers;
-- - trigger-only functions are not directly callable by anon/authenticated;
-- - authenticated keeps only the RPC/helper EXECUTE grants needed by the
--   current RLS/policy and learner/teacher flows;
-- - no service-role secret is involved in browser execution.

-- Trigger-only / internal functions: never direct RPC entrypoints.
...
-- No anonymous execution of any authorization helper or governed RPC.
...
-- Authenticated execution is explicit for policy helpers and governed RPCs.
