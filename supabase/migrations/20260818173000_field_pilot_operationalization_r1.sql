...
-- Additive staging-first migration. No production-specific data.
-- Team context is shared; learner reflection and Passport remain individual.
...
begin
  ...
  version_hash := encode(extensions.digest(version_snapshot::text, 'sha256'), 'hex');
  ...
  insert into public.mission_versions (
    mission_id, version_no, snapshot, content_hash, created_by, published_at
  ) values (
    target_mission_id, next_version_no, version_snapshot, version_hash, mission_row.created_by, now()
...
create index if not exists pilot_cohorts_org_status_idx on public.pilot_cohorts(organization_id, status);
create index if not exists pilot_cohort_members_user_idx on public.pilot_cohort_members(user_id, status);
create index if not exists experience_teams_cohort_idx on public.experience_teams(cohort_id, status);
create index if not exists experience_team_members_user_idx on public.experience_team_members(user_id, status);
create index if not exists experience_team_artifacts_team_idx on public.experience_team_artifacts(team_id, created_at);
...
-- ---------------------------------------------------------------------------
-- Access and governed pilot operations
-- ---------------------------------------------------------------------------
...
as $$
  select exists (
    ...
    where t.id = target_team_id
      and (
        public.pansofie_is_active_org_member(c.organization_id, array['teacher', 'coordinator']::text[], target_user_id)
        or exists (
...
begin
  if nullif(btrim(coalesce(target_name, '')), '') is null then raise exception 'cohort name is required'; end if;
  if not public.pansofie_is_active_org_member(target_org_id, array['teacher', 'coordinator']::text[], auth.uid()) and not public.is_admin() then
  ...
  values (target_org_id, btrim(target_name), 'planned', target_starts_on, target_ends_on, auth.uid())
...
set search_path = public
as $$
declare cohort_row public.pilot_cohorts%rowtype; membership_id uuid;
begin
  if target_role not in ('learner', 'teacher', 'coordinator') then raise exception 'unsupported cohort role'; end if;
  ...
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, array['teacher', 'coordinator']::text[], auth.uid()) and not public.is_admin() then
    raise exception 'teacher/coordinator membership required';
  end if;
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, array[target_role]::text[], target_user_id) then
    raise exception 'target user must have matching active organization role';
  end if;
  insert into public.pilot_cohort_members (cohort_id, user_id, role, status, created_by)
  values (target_cohort_id, target_user_id, target_role, 'active', auth.uid())
  on conflict (cohort_id, user_id, role) do update set status = 'active', ended_at = null
  returning id into membership_id;
  return membership_id;
end;
$$;

create or replace function public.pansofie_create_experience_team(target_cohort_id uuid, target_name text)
returns uuid
language plpgsql
security definer
set search_path = public
as $$
declare cohort_row public.pilot_cohorts%rowtype; team_id uuid;
begin
  if nullif(btrim(coalesce(target_name, '')), '') is null then raise exception 'team name is required'; end if;
  select * into cohort_row from public.pilot_cohorts where id = target_cohort_id;
  if cohort_row.id is null then raise exception 'cohort not found'; end if;
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, array['teacher', 'coordinator']::text[], auth.uid()) and not public.is_admin() then
    raise exception 'teacher/coordinator membership required';
  ...
  values (target_cohort_id, btrim(target_name), 'active', auth.uid()) returning id into team_id;
  return team_id;
end;
$$;
...
set search_path = public
as $$
declare cohort_row public.pilot_cohorts%rowtype; member_id uuid; required_org_roles text[];
begin
  if target_role not in ('learner', 'facilitator') then raise exception 'unsupported team role'; end if;
  select c.* into cohort_row
  from public.experience_teams t join public.pilot_cohorts c on c.id = t.cohort_id
  where t.id = target_team_id;
  if cohort_row.id is null then raise exception 'team not found'; end if;
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, array['teacher', 'coordinator']::text[], auth.uid()) and not public.is_admin() then
    raise exception 'teacher/coordinator membership required';
  end if;
  required_org_roles := case when target_role = 'learner' then array['learner']::text[] else array['teacher', 'coordinator']::text[] end;
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, required_org_roles, target_user_id) then
    raise exception 'target user does not have the required organization role';
  end if;
  insert into public.experience_team_members (team_id, user_id, role, status, created_by)
  values (target_team_id, target_user_id, target_role, 'active', auth.uid())
  on conflict (team_id, user_id) do update set role = excluded.role, status = 'active', ended_at = null
...
set search_path = public
as $$
declare caller_id uuid := auth.uid(); existing_run_id uuid; new_run_id uuid; target_version_id uuid;
begin
  if caller_id is null then raise exception 'authentication required'; end if;
  if not public.pansofie_is_active_org_member(target_org_id, array['teacher', 'coordinator']::text[], caller_id) and not public.is_admin() then
    raise exception 'teacher/coordinator membership required';
  end if;
  if not public.pansofie_is_active_org_member(target_org_id, array['learner']::text[], target_learner_id) then
    raise exception 'target learner must be active in organization';
  end if;
  if not public.pansofie_has_processing_basis(target_learner_id, target_org_id, 'school_mission_assignment') then
    raise exception 'school_mission_assignment processing basis required';
  end if;
  if not exists (select 1 from public.missions m where m.id = target_mission_id and m.status = 'published') and not public.is_admin() then
  ...
  insert into public.mission_runs (mission_id, mission_version_id, user_id, organization_id, assigned_by, status)
  values (target_mission_id, target_version_id, target_learner_id, target_org_id, caller_id, 'assigned')
  returning id into new_run_id;
  return new_run_id;
end;
$$;

create or replace function public.pansofie_assign_pilot_team_mission(target_mission_id uuid, target_team_id uuid)
...
begin
  ...
  if team_row.id is null or team_row.status <> 'active' then raise exception 'active team required'; end if;
  select * into cohort_row from public.pilot_cohorts where id = team_row.cohort_id;
  if cohort_row.id is null or cohort_row.status not in ('planned', 'active') then raise exception 'planned/active cohort required'; end if;
  if not public.pansofie_is_active_org_member(cohort_row.organization_id, array['teacher', 'coordinator']::text[], auth.uid()) and not public.is_admin() then
    raise exception 'teacher/coordinator membership required';
  end if;
  if not exists (select 1 from public.missions m where m.id = target_mission_id and m.status = 'published') then
    raise exception 'published mission required';
  end if;
  if not exists (select 1 from public.experience_team_members tm where tm.team_id = target_team_id and tm.role = 'learner' and tm.status = 'active') then
    raise exception 'team must contain at least one active learner';
  end if;

  for learner_row in
    select tm.user_id from public.experience_team_members tm
    where tm.team_id = target_team_id and tm.role = 'learner' and tm.status = 'active'
    order by tm.created_at
  loop
    if not public.pansofie_has_processing_basis(learner_row.user_id, cohort_row.organization_id, 'school_mission_assignment') then
  ...
  loop
    ...
    if existing_run_id is null then
      insert into public.mission_runs (
        mission_id, mission_version_id, user_id, organization_id, assigned_by, cohort_id, team_id, status
      ) values (
        target_mission_id, target_version_id, learner_row.user_id, cohort_row.organization_id, auth.uid(), cohort_row.id, target_team_id, 'assigned'
...
-- Finalization uses immutable snapshot provenance when available.
...
declare
  ...
  snapshot_paths text[];
begin
  select * into run_row from public.mission_runs where id = target_run_id for update;
  if run_row.id is null then raise exception 'run not found'; end if;
  if run_row.status = 'completed' then
    select e.id into existing_experience_id from public.experiences e where e.run_id = target_run_id;
    return existing_experience_id;
  end if;
  if run_row.status <> 'submitted' then raise exception 'run must be submitted before finalization'; end if;
  if not public.pansofie_can_review_run(target_run_id, 'school_mission_review') and not public.is_admin() then
    raise exception 'mission review access required';
  end if;
  select r.reviewer_id into final_reviewer
  from public.experience_reviews r
  where r.run_id = target_run_id and r.review_scope = 'mission' and r.status = 'confirmed'
  order by r.updated_at desc limit 1;
  if final_reviewer is null then raise exception 'confirmed mission review required'; end if;
  if not exists (select 1 from public.experience_evidence e where e.run_id = target_run_id) then raise exception 'evidence required'; end if;
  select * into reflection_row from public.experience_reflections x where x.run_id = target_run_id;
  if reflection_row.id is null or nullif(btrim(coalesce(reflection_row.what_learned, '')), '') is null then raise exception 'completed reflection required'; end if;
  select * into mission_row from public.missions m where m.id = run_row.mission_id;
  if mission_row.id is null then raise exception 'mission not found'; end if;

  if run_row.mission_version_id is not null then
    select mv.snapshot into mission_snapshot from public.mission_versions mv where mv.id = run_row.mission_version_id;
  end if;
  if mission_snapshot is null then
    mission_snapshot := jsonb_build_object('title', mission_row.title, 'path_ids', to_jsonb(mission_row.path_ids), 'program_id', mission_row.program_id, 'lab_id', mission_row.lab_id);
  end if;
  ...
  if existing_experience_id is null then
    insert into public.experiences (
      run_id, mission_id, mission_version_id, cohort_id, team_id, user_id, title, path_ids, program_id, lab_id, impact_summary, occurred_at
    ) values (
      run_row.id, mission_row.id, run_row.mission_version_id, run_row.cohort_id, run_row.team_id, run_row.user_id,
      coalesce(mission_snapshot->>'title', mission_row.title), snapshot_paths,
  ...
  insert into public.portfolio_items (experience_id, user_id, title, summary, visibility, verified_by, verified_at)
  values (
    experience_id, run_row.user_id, coalesce(mission_snapshot->>'title', mission_row.title),
    nullif(btrim(coalesce(reflection_row.what_learned, '')), ''), 'private', final_reviewer, now()
  ) on conflict (experience_id) do nothing;

  update public.mission_runs set status = 'completed', completed_at = now() where id = target_run_id;
...
create policy "mission_versions_select_authenticated" on public.mission_versions for select to authenticated using (true);

drop policy if exists "pilot_cohorts_select_org_members" on public.pilot_cohorts;
create policy "pilot_cohorts_select_org_members" on public.pilot_cohorts for select to authenticated
using (public.is_admin() or public.pansofie_is_active_org_member(organization_id, null, auth.uid()));

drop policy if exists "pilot_cohorts_write_staff" on public.pilot_cohorts;
create policy "pilot_cohorts_write_staff" on public.pilot_cohorts for all to authenticated
...
create policy "pilot_cohort_members_select_org_members" on public.pilot_cohort_members for select to authenticated
using (public.is_admin() or exists (
  select 1 from public.pilot_cohorts c where c.id = cohort_id and public.pansofie_is_active_org_member(c.organization_id, null, auth.uid())
));

drop policy if exists "pilot_cohort_members_write_staff" on public.pilot_cohort_members;
create policy "pilot_cohort_members_write_staff" on public.pilot_cohort_members for all to authenticated
using (public.is_admin() or exists (
  select 1 from public.pilot_cohorts c where c.id = cohort_id and public.pansofie_is_active_org_member(c.organization_id, array['teacher', 'coordinator']::text[], auth.uid())
))
...
create policy "experience_teams_select_org_members" on public.experience_teams for select to authenticated
using (public.is_admin() or exists (
  select 1 from public.pilot_cohorts c where c.id = cohort_id and public.pansofie_is_active_org_member(c.organization_id, null, auth.uid())
));

drop policy if exists "experience_teams_write_staff" on public.experience_teams;
create policy "experience_teams_write_staff" on public.experience_teams for all to authenticated
using (public.is_admin() or exists (
  select 1 from public.pilot_cohorts c where c.id = cohort_id and public.pansofie_is_active_org_member(c.organization_id, array['teacher', 'coordinator']::text[], auth.uid())
))
with check (public.is_admin() or exists (
...
create policy "experience_team_members_select_access" on public.experience_team_members for select to authenticated
...
create policy "experience_team_members_write_staff" on public.experience_team_members for all to authenticated
using (public.is_admin() or exists (
  select 1 from public.experience_teams t join public.pilot_cohorts c on c.id = t.cohort_id
  where t.id = team_id and public.pansofie_is_active_org_member(c.organization_id, array['teacher', 'coordinator']::text[], auth.uid())
))
with check (public.is_admin() or exists (
  select 1 from public.experience_teams t join public.pilot_cohorts c on c.id = t.cohort_id
  where t.id = team_id and public.pansofie_is_active_org_member(c.organization_id, array['teacher', 'coordinator']::text[], auth.uid())
...
create policy "experience_team_artifacts_select_access" on public.experience_team_artifacts for select to authenticated
using (public.is_admin() or public.pansofie_can_access_team(team_id, auth.uid()));

drop policy if exists "experience_team_artifacts_insert_access" on public.experience_team_artifacts;
create policy "experience_team_artifacts_insert_access" on public.experience_team_artifacts for insert to authenticated
with check (created_by = auth.uid() and (public.is_admin() or public.pansofie_can_access_team(team_id, auth.uid())));

drop policy if exists "experience_team_artifacts_delete_creator_or_staff" on public.experience_team_artifacts;
create policy "experience_team_artifacts_delete_creator_or_staff" on public.experience_team_artifacts for delete to authenticated
using (
  created_by = auth.uid()
  or public.is_admin()
  or exists (
...
(
  'zlepsi-svou-skolu', 'Zlepši svou školu',
  ...
  'school', 'community', array['tvorba-reseni', 'vztahy-spoluprace', 'obcanstvi-prinos']::text[], 'published', 240,
  ...
  '{}'::jsonb, '{"target_review_minutes_per_team":5}'::jsonb
),
(
  'digitalni-most', 'Digitální most',
  ...
  'school', 'community', array['poznani-mysleni', 'vztahy-spoluprace', 'obcanstvi-prinos']::text[], 'published', 180,
...
(
  'circular-challenge', 'Circular Challenge',
  ...
  'school', 'nature', array['priroda-udrzitelnost', 'tvorba-reseni', 'samostatnost-podnikavost']::text[], 'published', 240,
