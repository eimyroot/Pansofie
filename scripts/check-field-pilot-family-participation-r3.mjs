...
const [migration, verification, service, page, app, layout, doc] = await Promise.all([
...
assert.ok(sql.includes("revoke all on table public.family_contributions from authenticated"), "direct authenticated family table access must be revoked");
...
assert.ok(sql.includes("'school_mission_review'"), "staff family inbox must preserve school review basis");
...
assert.ok(sql.includes("revoke execute on function public.pansofie_can_guardian_participate_in_run(uuid) from anon"), "private helper anon revoke missing");
assert.ok(sql.includes("revoke execute on function public.pansofie_can_guardian_participate_in_run(uuid) from authenticated"), "private helper authenticated revoke missing");
